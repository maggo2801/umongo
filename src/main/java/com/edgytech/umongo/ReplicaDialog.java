...
public class ReplicaDialog extends FormDialog {
  enum Item {
    host, arbiterOnly, hidden, priority, votes, tags, slaveDelay, ignoreIndexes
  }
  ...
  public void updateFromReplicaConfig(final BasicDBObject member) {
    // reset
    xmlLoadCheckpoint();
  ...
  public BasicDBObject getReplicaConfig(final int id) {
    final BasicDBObject member = new BasicDBObject("_id", id);
    member.put("host", getStringFieldValue(Item.host));
    if (getBooleanFieldValue(Item.arbiterOnly)) {
      member.put("arbiterOnly", true);
    }
    if (getBooleanFieldValue(Item.hidden)) {
      member.put("hidden", true);
    }
    if (getBooleanFieldValue(Item.ignoreIndexes)) {
      member.put("buildIndexes", false);
    }

    final double priority = getDoubleFieldValue(Item.priority);
    if (priority != 1.0) {
      member.put("priority", priority);
    }
    final int slaveDelay = getIntFieldValue(Item.slaveDelay);
    if (slaveDelay > 0) {
      member.put("slaveDelay", slaveDelay);
    }
    final int votes = getIntFieldValue(Item.votes);
    if (votes != 1) {
      member.put("votes", votes);
    }

    final DBObject tags = ((DocBuilderField) getBoundUnit(Item.tags)).getDBObject();
    if (tags != null) {
      member.put("tags", tags);
    }
    return member;
  }
}
