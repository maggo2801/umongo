...
public class MongoPanel extends BasePanel implements EnumListener<Item> {
  ...
  enum Item {
    ...
    icon, version, connectPoint, masterServer, seedServers, activeServers, queryOptions, writeConcern, readPreference, serverStatus, readWriteOptions, refresh, close, createDB, createDbName, authenticate, authUser, authPassword, maxObjectSize, cloneDB, cloneDBHost, cloneDBFrom, cloneDBTo, cloneDBSlaveOk, currentOps, currentOpsQuery, killOp, killOpId, isMaster, summarizeData
  }
  ...
  private String getServerAddressString(final ServerAddress addr) {
    String ip = "unknown";
    try {
  ...
  protected void updateComponentCustom(final JPanel old) {
    try {
      ...
      final ServerAddress master = mongo.getAddress();
      if (master != null) {
      ...
      html = "<html>";
      for (final ServerAddress addr : addrs) {
        // String ports = MongoUtils.makeInfoString("used",
        // mongo.getConnector().getDBPortPool(addr).inUse(),
        // "available", mongo.getConnector().getDBPortPool(addr).available(),
        // "created", mongo.getConnector().getDBPortPool(addr).everCreated());
        final String txt = getServerAddressString(addr);
        html += txt + "<br/>";
      }
